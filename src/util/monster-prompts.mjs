...
export function createStatblockPrompts(options) {
  ...
  if (template.damage_resistances){
    resistanceAndImmunitiesPrompt = `The creature has 1-3 damage_resistances. The creature has 0 immunities.`
  }
  if (template.damage_immunities){
    resistanceAndImmunitiesPrompt = `The creature has 1-3 damage_immunities. The creature has 0 resistances.`
  }
  if (template.resistances && template.immunities){
  ...
  const part2Obj = {
    actions: [
      {
        name: "Example Action: Longsword",
        description:
          'Example Description: Melee Weapon Attack: +10 to hit, reach 5 ft., one target. Hit: 22 (3d10 + 6) slashing damage.',
      },
      ...template.actions
    ],
  };
  ...
  return {
    ...
    ${savingThrowsPrompt}. ${resistanceAndImmunitiesPrompt}. Remember that the only possible damage types are from this list: Acid, Bludgeoning, Cold, Fire, Force, Lightning, Necrotic, Piercing, Poison, Psychic, Radiant, Slashing, Thunder as well as "Bludgeoning, Slashing and Piercing from non-magical weapons". Finally condition_immunities can only be selected from this list: Blinded, Charmed, Deafened, Frightened, Grappled, Incapacitated, Paralyzed, Petrified, Poisoned, Prone, Restrained, Stunned, Unconscious`,
    
    part2: `Please give me the second part of a D&D statblock in the following format. Use the same number of actions and abilities (except the example action) and use all the info provided in the description:
    
    ${JSON.stringify(part2Obj, null, "\t")}
    
    With the above format in mind, please generate the second part of a statblock for ${monsterName}. This creature has a challenge_rating ${challengeRatingObj.CR}. The attack_bonus for each attack should be ${challengeRatingObj.attack_bonus}. Please use the exact damage and saving throws for each action provided above. Provide damage types and flavor. ${multiattackString} Actions (Action 1, Action 2, etc) should be replaced with their actual generated names. Actions should not repeat any content from the previously generated abilities array. Don't include passive abilities. Finally, if an action doesn't fit thematically with ${monsterName} feel free to modify but try to keep damage and DC saves the same.`,
  };
}
