<template>
  ...
  const generateBookTitles = async () => {
    const prompt = `Generate a list of 5 book titles. The books are located in a ${location.value} in the ${setting.value} setting and the books span various genres including ${genre.value}.`;
